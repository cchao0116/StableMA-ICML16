...
public abstract class FactorRecmmder extends Recommender {
	...
	public FactorRecmmder(Configures conf, boolean[] acc_ufi, boolean[] acc_ifi, Map<String, Plugin> plugins) {
		runtimes = new RuntimeEnv(conf);
		runtimes.plugins = plugins;
	 ...
	 */
	/**
	 * @see code.sma.recmmd.Recommender#prepare_runtimes(code.sma.core.AbstractMatrix,
	...
	protected void finish_round() {
		...
		if (runtimes.showProgress && runtimes.itest != null) {
			...
			LoggerUtil.info(runningLogger, String.format("%d\t%.6f", runtimes.round, runtimes.currErr));
		}
