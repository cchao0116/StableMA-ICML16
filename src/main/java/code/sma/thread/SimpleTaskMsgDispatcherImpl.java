 ...
 * 
 * @author Chao.Chen
 * @version $Id: SimpleTaskMsgDispatcher.java, v 0.1 2016年9月27日 下午12:24:16
 *          Chao.Chen Exp $
 */
public class SimpleTaskMsgDispatcherImpl implements TaskMsgDispatcher {
	...
	protected final static Logger normalLogger = Logger.getLogger(LoggerDefineConstant.SERVICE_NORMAL);
	...
	public SimpleTaskMsgDispatcherImpl(Configures conf) {
		...
		for (Object k : conf.keySet()) {
			...
			if (key.endsWith("ARR")) {
				suffArrKeys.add(StringUtil.reverse(StringUtil.reverse(key).replace("RRA", "EULAV")));
			...
			while (!nodes.isEmpty()) {
				...
				for (int l = 0; l < lastLayerWidth; l++) {
					...
					for (int c = 0; c < maxLayer - 1; c++) {
						rce.setDouble(suffArrKeys.get(c), (double) suffArrVals.get(c).floatValue(nodes.get(c)));
					}
					rce.setDouble(suffArrKeys.get(nextLayer), (double) suffArrVals.get(nextLayer).floatValue(l));
	...
	/**
	 * @see code.sma.thread.TaskMsgDispatcher#reduce(java.lang.Object,
	 *      code.sma.core.AbstractMatrix, code.sma.core.AbstractMatrix)
	 */
	@Override
